...
try {
    ...
    $leaveRequestMigrations = [
        "ALTER TABLE leave_requests ADD COLUMN reject_comment TEXT NULL",
        "ALTER TABLE leave_requests ADD COLUMN processed_by INT NULL",
        "ALTER TABLE leave_requests ADD COLUMN processed_at DATETIME NULL",
        "ALTER TABLE leave_requests ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP",
        "CREATE INDEX idx_leave_user_date ON leave_requests(user_id, leave_date)",
        "CREATE INDEX idx_leave_status ON leave_requests(status)",
        "CREATE INDEX idx_leave_type ON leave_requests(leave_type)",
    ];
    foreach ($leaveRequestMigrations as $sql) {
        try { $db->query($sql); } catch (Exception $e) {}
    }
...
function canUseLeaveRequestTest() {
    return isset($_SESSION['username']) && $_SESSION['username'] === 'stafftest';
}

function normalizeLeaveType($value) {
    $allowed = ['public_holiday', 'paid_leave'];
    return in_array($value, $allowed, true) ? $value : '';
}

function getLeaveUsage($userId, $yearMonth, $year, $excludeRequestId = 0) {
    global $db;
    $publicParams = [$userId, $yearMonth];
    $paidParams = [$userId, $year];
    $excludeSql = '';
    if ($excludeRequestId > 0) {
        $excludeSql = " AND id <> ?";
        $publicParams[] = $excludeRequestId;
        $paidParams[] = $excludeRequestId;
    }

    $publicHoliday = $db->fetch(
        "SELECT COUNT(*) AS count FROM leave_requests
         WHERE user_id = ? AND leave_type = 'public_holiday'
           AND status IN ('pending', 'approved')
           AND DATE_FORMAT(leave_date, '%Y-%m') = ?{$excludeSql}",
        $publicParams
    );
    $paidLeave = $db->fetch(
        "SELECT COUNT(*) AS count FROM leave_requests
         WHERE user_id = ? AND leave_type = 'paid_leave'
           AND status IN ('pending', 'approved')
           AND YEAR(leave_date) = ?{$excludeSql}",
        $paidParams
    );

    $publicUsed = (int)($publicHoliday['count'] ?? 0);
    $paidUsed = (int)($paidLeave['count'] ?? 0);
    return [
        'publicHoliday' => [
            'used' => $publicUsed,
            'limit' => 6,
            'remaining' => max(0, 6 - $publicUsed)
        ],
        'paidLeave' => [
            'used' => $paidUsed,
            'limit' => 10,
            'remaining' => max(0, 10 - $paidUsed)
        ]
    ];
}
...
<body>
    ...
    case 'leave-requests':
        ...
        if (!canUseLeaveRequestTest()) {
            error('休暇申請テストはstafftestのみ利用できます', 403);
        }
        ...
        if ($method === 'GET') {
            ...
            $year = (int)($_GET['year'] ?? substr($yearMonth, 0, 4));
            ...
            respond([
                'requests' => $requests,
                'summary' => getLeaveUsage((int)$_SESSION['user_id'], $yearMonth, $year)
            ]);
        }

        if ($method === 'POST') {
            $leaveDate = trim($input['leave_date'] ?? $_GET['leave_date'] ?? '');
            $leaveType = normalizeLeaveType($input['leave_type'] ?? $_GET['leave_type'] ?? '');
            $reason = trim($input['reason'] ?? $_GET['reason'] ?? '');

            if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $leaveDate)) {
                error('日付を指定してください');
            }
            $dateObj = DateTime::createFromFormat('Y-m-d', $leaveDate);
            if (!$dateObj || $dateObj->format('Y-m-d') !== $leaveDate) {
                error('日付の形式が不正です');
            }
            if (!$leaveType) {
                error('休暇種別を指定してください');
            ...
            $duplicate = $db->fetch(
                "SELECT id FROM leave_requests
                 ...
                 LIMIT 1",
                [$_SESSION['user_id'], $leaveDate]
            );
            if ($duplicate) {
                error('この日付は既に休暇申請中、または承認済みです');
            }

            $yearMonth = $dateObj->format('Y-m');
            $year = (int)$dateObj->format('Y');
            $usage = getLeaveUsage((int)$_SESSION['user_id'], $yearMonth, $year);
            if ($leaveType === 'public_holiday' && $usage['publicHoliday']['used'] >= $usage['publicHoliday']['limit']) {
                error('公休はこの月の上限6回に達しています');
            }
            if ($leaveType === 'paid_leave' && $usage['paidLeave']['used'] >= $usage['paidLeave']['limit']) {
                error('有給はこの年の上限10日に達しています');
            }

            $id = $db->insert(
                "INSERT INTO leave_requests (user_id, leave_date, leave_type, reason, status, created_at)
                 VALUES (?, ?, ?, ?, 'pending', NOW())",
                [$_SESSION['user_id'], $leaveDate, $leaveType, $reason]
            );

            logAudit('create', 'leave_request', $id, $_SESSION['name'] ?? '', [
                'leave_date' => $leaveDate,
                'leave_type' => $leaveType,
                'reason' => $reason
            ]);

            respond(['id' => $id, 'message' => '休暇申請を送信しました']);
        }
        break;

    case 'leave-requests-admin':
        checkAdmin();

        if ($method === 'GET') {
            $status = $_GET['status'] ?? 'pending';
            $sql = "SELECT lr.*, u.name AS user_name, u.username
                    FROM leave_requests lr
                    JOIN users u ON lr.user_id = u.id
                    WHERE u.username = 'stafftest'";
            $params = [];
            if ($status) {
                $sql .= " AND lr.status = ?";
                $params[] = $status;
            ...
            respond($db->fetchAll($sql, $params));
        }
        break;

    case 'leave-requests-count':
        checkAdmin();
        ...
        if ($method === 'GET') {
            $count = $db->fetch(
                "SELECT COUNT(*) AS count
                 FROM leave_requests lr
                 JOIN users u ON lr.user_id = u.id
                 WHERE lr.status = 'pending' AND u.username = 'stafftest'"
            );
            respond(['count' => (int)($count['count'] ?? 0)]);
        }
        break;
    ...
    case 'leave-request-approve':
        ...
        if ($method === 'POST') {
            $id = (int)($input['id'] ?? $_GET['id'] ?? $_REQUEST['id'] ?? 0);
            if ($id <= 0) {
                error('Invalid request ID');
            }

            $request = $db->fetch(
                "SELECT lr.*, u.username, u.name AS user_name
                 FROM leave_requests lr
                 JOIN users u ON lr.user_id = u.id
                 WHERE lr.id = ?",
                [$id]
            ...
            if (!$request) {
                error('休暇申請が見つかりません', 404);
            }
            if ($request['username'] !== 'stafftest') {
                error('休暇申請テストはstafftestのみ処理できます', 403);
            }
            if ($request['status'] !== 'pending') {
                error('この申請は既に処理済みです');
            }

            $dateObj = new DateTime($request['leave_date']);
            $usage = getLeaveUsage((int)$request['user_id'], $dateObj->format('Y-m'), (int)$dateObj->format('Y'), $id);
            if ($request['leave_type'] === 'public_holiday' && $usage['publicHoliday']['used'] >= $usage['publicHoliday']['limit']) {
                error('公休はこの月の上限6回に達しています');
            }
            if ($request['leave_type'] === 'paid_leave' && $usage['paidLeave']['used'] >= $usage['paidLeave']['limit']) {
                error('有給はこの年の上限10日に達しています');
            }

            $db->update(
                "UPDATE leave_requests SET status = 'approved', processed_by = ?, processed_at = NOW() WHERE id = ?",
                [$_SESSION['user_id'], $id]
            );

            logAudit('approve', 'leave_request', $id, $request['user_name'], [
            ...
            respond(['message' => '休暇申請を承認しました']);
        }
        break;

    case 'leave-request-reject':
        checkAdmin();

        if ($method === 'POST') {
            $id = (int)($input['id'] ?? $_GET['id'] ?? $_REQUEST['id'] ?? 0);
            $comment = trim($input['comment'] ?? $_GET['comment'] ?? '');
            if ($id <= 0) {
                error('Invalid request ID');
            }
            ...
            $db->update(
                "UPDATE leave_requests SET status = 'rejected', reject_comment = ?, processed_by = ?, processed_at = NOW() WHERE id = ?",
                [$comment, $_SESSION['user_id'], $id]
            );

            logAudit('reject', 'leave_request', $id, $request['user_name'], [
                'leave_date' => $request['leave_date'],
                'leave_type' => $request['leave_type'],
                'comment' => $comment,
                'rejected_by' => $_SESSION['user_id'],
                'rejected_by_name' => $_SESSION['name'] ?? ''
            ]);

            respond(['message' => '休暇申請を却下しました']);
        }
        break;
