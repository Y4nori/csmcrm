...
<body>
    ...
    case 'timecard-request-approve':
        ...
        if ($method === 'POST') {
            ...
            // 時刻値を正規化（HH:MM:SS形式に変換）
            $clockIn = $request['clock_in'];
            $clockOut = $request['clock_out'];

            // 空文字やNULLの場合はNULLに統一
            if (empty($clockIn)) $clockIn = null;
            if (empty($clockOut)) $clockOut = null;

            // HH:MM形式の場合は:00を付与
            if ($clockIn && preg_match('/^\d{2}:\d{2}$/', $clockIn)) {
                $clockIn = $clockIn . ':00';
            }
            ...
            try {
                ...
                if ($existing) {
                    ...
                    if ($clockIn !== null) {
                        ...
                        $updateParams[] = $clockIn;
                        $updateFields[] = "clock_in_type = 'corrected'";
                    }
                    if ($clockOut !== null) {
                        $updateFields[] = "clock_out = ?";
                        $updateParams[] = $clockOut;
                        $updateFields[] = "clock_out_type = 'corrected'";
                    }

                    if (!empty($updateFields)) {
                        $updateParams[] = $existing['id'];
                        $db->update(
                            "UPDATE timecards SET " . implode(', ', $updateFields) . " WHERE id = ?",
                            $updateParams
                        );
                    }
                } else {
                    $db->insert(
                        ...
                        [$request['user_id'], $request['work_date'], $clockIn, $clockOut]
                    );
                }

                respond(['message' => '申請を承認しました']);
            } catch (Exception $e) {
                // 承認処理が途中で失敗した場合、申請ステータスを元に戻す
                try {
                    $db->update(
                ...
                error('承認処理に失敗しました: ' . $e->getMessage());
            }
        }
        break;
